...
const plc = new M221Plc(config.plc);
...
let state = {
  ...
  source: { host: config.plc.host, port: config.plc.port, unitId: config.plc.unitId },
  timestamp: null,
  latencyMs: null,
  error: null,
  data: null
...
server.listen(config.listen.port, config.listen.host, () => {
  ...
  console.log(`PLC   : ${config.plc.host}:${config.plc.port} unit ${config.plc.unitId}`);
